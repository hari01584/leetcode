...
class Solution {
    ...
    public int brokenBinSearch(int start, int end, int target, int[] nums) {
        while (start <= end) {
            ...
            // If all three elements are same, then we cannot compare which one is sorted or not just on basis on index and values
            // so loop one ahead and decrease window in both sides until this situation is false!
    ...
    public Boolean search(int[] nums, int target) {
        ...
        int index = brokenBinSearch(0, nums.length - 1, target, nums);
