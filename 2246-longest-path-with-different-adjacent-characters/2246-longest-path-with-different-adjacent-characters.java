/*
explanation: longest path using recursion! The program works by maintaining three parameters (char, count1, count2) for each node at each point! It works by finding the loop thru path in each of case and returning the maximum of two!

testcase:
[-1,0,0,1,1,2]
"abacbe" -> Works

Time & Space Complexity: O(n) & O(n): Here both time and space complexity is linear because recursion is used!
*/
class Solution {
    class R {
        char last;
